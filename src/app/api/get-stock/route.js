...
export async function GET(req, res) {
  try {
    ...
    const sessionUser = await SessionUser.findOne({ user_email: user.email }).populate('stocks');
    ...
    // })
    const getstocks = await Stock.find({ _id : sessionUser.stocks })
    // Return the user's stock data
    return NextResponse.json({ stocks: getstocks}, { status: 200 });
